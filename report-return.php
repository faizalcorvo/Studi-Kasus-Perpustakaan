...
$query = "SELECT pengembalian.*, tblbuku.judulBuku, tblbuku.gambar, peminjaman.tgl_pinjam, peminjaman.tgl_tempo,pengembalian.id_kembali, pengembalian.tgl_kembali, member.fname FROM peminjaman JOIN tblbuku ON tblbuku.no_pustaka = peminjaman.no_pustaka JOIN member ON member.no_id = peminjaman.no_id JOIN pengembalian ON peminjaman.id_pinjam = pengembalian.id_pinjam"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
// session start
if (!empty($_SESSION)) {
} else {
    session_start();
                                    ...
                                    <tbody>
                                        ...
                                        if ($row > 0) { // Jika jumlah data lebih dari 0 (Berarti jika data ada)
                                            while ($data = mysqli_fetch_array($sql)) { // Ambil semua data dari hasil eksekusi $sql
                                                $tgl = date('d-m-Y', strtotime($data['id_kembali']));
                                        ?>
                                                <tr>
